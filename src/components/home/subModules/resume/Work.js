...
const Work = props => {
	...
	return (
		<div key={work.id}>
			...
			<div className="resume__logo" style={{backgroundImage: `url("${work.logo}")`}}/>
			...
			<p className="resume__accomplishment">Accomplishments:</p>
			{work.accomplishments.map((acc, index) => {
				return (
					<div key={index}>
						<span>&bull;</span> <em className="date">{acc}</em>
					</div>
				);
			})}
			<br />
